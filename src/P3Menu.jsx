...
import { ayudaMenu, marcaMenu, menuPrincipal } from "./siteContent";

const ITEMS = menuPrincipal;
...
export default function P3Menu({ onNavigate }) {
  ...
  const navigateWrap = useCallback((direction) => {
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "Enter")     onNavigate?.(ITEMS[active].pagina);
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [active, onNavigate, navigateWrap]);
  ...
  return (
    <>
      <style>{`
        ...
        color: rgba(0, 0, 0, 0.86); text-shadow: 1px 1px 0 rgba(60, 226, 255, 0.3);
        }
      ...
      <div className="p3-overlay">
        <div className="p3-name-tag">
          <span>{marcaMenu.linea1}</span>
          <span>{marcaMenu.linea2}</span>
        </div>
        ...
        <nav className="p3-menu">
          {ITEMS.map((item, i) => {
            ...
            const estW = item.etiqueta.length * item.fontSize * 0.6 + 80;
            ...
            return (
              <a
                ...
                onClick={(e) => { e.preventDefault(); onNavigate?.(item.pagina); }}
        ...
        <div className={`p3-hint ${mounted ? "mounted" : ""}`}>
          <div className="p3-hint-row"><span className="p3-hint-key">↑↓</span><span>{ayudaMenu.seleccionar}</span></div>
          <div className="p3-hint-row"><span className="p3-hint-key">↵</span><span>{ayudaMenu.confirmar}</span></div>
        </div>
      </div>
    </>
  );
